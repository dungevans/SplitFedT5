            ...
            if start:
                if self.load_parameters and register:
                    if os.path.exists(filepath):
                        ...
                        # --- ĐOẠN THAY THẾ ---
# 1. Khởi tạo model dựa trên loại kiến trúc
                        if self.model_name == 'T5':
                            full_model_path = "philschmid/flan-t5-base-samsum" 
                            model = klass(model_name=full_model_path, layer_id=layer_id)
                        else:
                            if layer_id == 1:
                        ...
                        # 2. Lấy state_dict trống của model vừa khởi tạo
                        state_dict = model.state_dict()

                        # 3. Nạp tham số từ file checkpoint (full_state_dict) vào state_dict của model
                        if self.model_name == 'T5':
                            # T5 chia Encoder/Decoder nên không cần dịch chuyển key (mapping trực tiếp)
                            for key in state_dict.keys():
                                state_dict[key] = full_state_dict[key]
                        else:
                            # Logic cũ cho GPT2/Llama (có xử lý dịch chuyển ID block cho Layer 2)
                            if layer_id == 2:
                        ...
                        src.Log.print_with_color(f"Load parameters for {self.model_name} (Layer {layer_id}) successfully", "green")
