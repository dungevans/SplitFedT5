...
from src.fine_tune.T5 import Ft_T5
    ...
    def response_message(self, body):
        ...
        if action == "START":
            ...
            if model_name == 'GPT2':
                ...
                self.model_train = Ft_T5(self.client_id, self.layer_id, self.channel, self.device)
            ...
            if fine_tune_config['name'] == 'LoRA':
                if model_name == 'GPT2':
                    ...
                    peft_config = LoraConfig(
                        ...
                        target_modules=["q", "v"] # Target vào attention của T5
                    )
                else:
                    peft_config = None
            else:
           ...
           # ... (Phần code cấu hình klass và peft_config bên trên)
            ...
            if model_name == 'T5':
                # Lấy đường dẫn model từ server hoặc dùng mặc định
                full_model_path = self.response.get("model_path", "philschmid/flan-t5-base-samsum")
                if self.layer_id == 1:
                    model = klass(model_name=full_model_path, layer_id=1)
                else:
                    model = klass(model_name=full_model_path, layer_id=2)
            else:
                # Giữ nguyên logic cho các model cũ (GPT2, Llama, Bert)[cite: 1, 2]
                if self.layer_id == 1:
                    model = klass(layer_id=1, n_block=cut_layers)
                else:
                    model = klass(layer_id=2, n_block=total_block-cut_layers)
