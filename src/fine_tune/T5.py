...
class Ft_T5:
    ...
    def send_intermediate_output(self, data_id, output, attention_mask, labels, trace):
        ...
        traces = (trace + [self.client_id]) if trace else [self.client_id]
        ...
        self.channel.basic_publish(exchange='', routing_key=forward_queue_name, body=message)
    ...
    def send_gradient(self, data_id, gradient, trace):
        ...
        self.channel.basic_publish(exchange='', routing_key=backward_queue_name, body=message)

    def send_to_server(self, message):
        self.channel.queue_declare('rpc_queue', durable=False)
        self.channel.basic_publish(exchange='', routing_key='rpc_queue', body=pickle.dumps(message))
    ...
    def first_layer(self, model, lr, weight_decay, clip_grad_norm, control_count=1, train_loader=None):
        ...
        with tqdm(total=len(train_loader), desc="Processing", unit="step") as pbar:
            while True:
                ...
                method_frame, _, body = self.channel.basic_get(queue=backward_queue_name, auto_ack=True)
        ...
        while True:
            _, _, body = self.channel.basic_get(queue=broadcast_queue_name, auto_ack=True)
            if body:
                received_data = pickle.loads(body)
    ...
    def last_layer(self, model, lr, weight_decay, clip_grad_norm):
        ...
        while True:
            method_frame, _, body = self.channel.basic_get(queue=forward_queue_name, auto_ack=True)
            if method_frame and body:
                optimizer.zero_grad()
                received_data = pickle.loads(body)
                intermediate_output = torch.tensor(received_data["data"], requires_grad=True).to(self.device)
                ...
                output, _ = model(encoder_hidden_states=intermediate_output, attention_mask=attention_mask, labels=labels)
                ...
                self.send_gradient(data_id, intermediate_output.grad, trace)
            else:
                broadcast_queue_name = f'reply_{self.client_id}'
                _, _, body = self.channel.basic_get(queue=broadcast_queue_name, auto_ack=True)
                if body:
